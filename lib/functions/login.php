...
if (empty($username)) {
    echo json_encode(array('status' => 'error', 'message' => 'نام کاربری  را وارد کنید.'));
    exit();
}
if (empty($package_id)) {
...
$result = api_function(API_CLOUD_HOST, 'login', API_CLOUD_TOKEN, $input);
...
if ($result['status'] == 'success') {
    if ($result['response']['client_two_step_login'] == true) {
        $flow_token = $result['response']['flow_token'];
  
        echo json_encode(array('status' => 'success', 'message' => $result['message'], 'response' => $result['response']));
        exit();
    }
